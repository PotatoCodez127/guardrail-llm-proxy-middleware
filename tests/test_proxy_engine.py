...
def mock_settings():
    return ProxySettings(
        ollama_api_base_url="[https://mock.ollama.com](https://mock.ollama.com)",
...
async def test_evaluate_safety_malformed_json_fallback(mock_settings):
    ...
    with patch.object(engine.client, 'chat', new_callable=AsyncMock) as mocked_chat:
        ...
        # Verify fallback logic kicks in cleanly without crashing
        assert result.safe is False
        assert "Structural non-conformance" in result.reason